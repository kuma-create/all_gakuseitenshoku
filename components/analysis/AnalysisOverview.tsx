      ...
      >
        <Card className="p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-2">自己分析の進捗</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <motion.div 
              className="text-center p-4 sm:p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border border-green-200"
            ...
            <motion.div 
              className="text-center p-4 sm:p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200"
              whileHover={{ scale: 1.02, y: -2 }}
              transition={{ type: "spring", stiffness: 300 }}
            >
            ...
            <motion.div 
              className="text-center p-4 sm:p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border border-purple-200"
              whileHover={{ scale: 1.02, y: -2 }}
              transition={{ type: "spring", stiffness: 300 }}
            >
        ...
        >
          <Card className="p-6 border-2 border-dashed border-primary/30 bg-gradient-to-r from-primary/5 to-primary/10">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-foreground">次におすすめのアクション</h3>
                <p className="text-sm text-muted-foreground">継続は力なり - 少しずつでも進めていきましょう</p>
              </div>
            </div>
            
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-card rounded-xl border gap-4">
              <div className="flex items-start space-x-4 flex-1">
                <div className={`p-3 rounded-xl bg-gradient-to-br ${getColorClasses(nextTool.color).bg} border ${getColorClasses(nextTool.color).border}`}>
                  <nextTool.icon className={`w-6 h-6 ${getColorClasses(nextTool.color).icon}`} />
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-foreground">{nextTool.title}</h4>
                  <p className="text-sm text-muted-foreground mb-2">{nextTool.description}</p>
                  <div className="flex flex-wrap items-center gap-3">
                    <Badge variant="outline" className="text-xs">
                      <Clock className="w-3 h-3 mr-1" />
                      {nextTool.estimatedTime}
                    </Badge>
                    <Badge variant="outline" className={`text-xs ${getDifficultyColor(nextTool.difficulty)}`}>
                      {nextTool.difficulty}
                    </Badge>
                    {nextTool.badge && (
            ...
            >
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className={`p-3 rounded-xl bg-gradient-to-br ${colors.bg} border ${colors.border} relative`}>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-foreground">進捗</span>
                  ...
                  <div>
                    <div className="text-xs font-medium text-muted-foreground mb-2">期待できる成果:</div>
