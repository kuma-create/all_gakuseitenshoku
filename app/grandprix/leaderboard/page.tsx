...
export default function WebTestPage() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      else setAnswers(data as AnswerRow[])
      ...
      if (sessErr) toast({ description: sessErr.message })
        ...
        setDeadline(new Date(new Date(sess.started_at).getTime() + 40 * 60 * 1000))
      }
      setLoading(false)
    })()
  ...
  if (!currentAnswer) return <p className="p-4">問題が見つかりません</p>
      ...
      <header className="sticky top-0 z-10 bg-white shadow-sm">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-2">
            <LazyImage src="/placeholder.svg?height=32&width=32" alt="学生転職ロゴ" width={32} height={32} className="h-8 w-8" />
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 rounded-full bg-emerald-100 px-4 py-2">
              ...
              <span className="text-sm font-medium text-emerald-700">残り時間: {remainStr}</span>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mb-6">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-4">
              ...
              <Progress value={progressPct} className="h-2 w-32 bg-gray-200" indicatorClassName="bg-emerald-500" />
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <Timer className="h-4 w-4" />
                <span>経過時間: {((40*60 - remaining)/60).toFixed(0).padStart(2,'0') }:{((40*60 - remaining)%60).toString().padStart(2,'0')}</span>
              </div>
            </div>
          </div>
          ...
          <Card className="mb-6">
            ...
            <CardFooter className="flex justify-between border-t bg-gray-50 p-4">
              ...
              <Button variant="outline" className="gap-1" onClick={() => toast({ description: "Saved" })}>
          ...
          <div className="flex justify-between">
            <div className="flex flex-wrap gap-2">
              {answers.map((_, idx) => (
                <Button
                  key={idx}
                  variant={idx === current ? "default" : idx < current ? "secondary" : "outline"}
                  size="sm"
                  className={idx === current ? "bg-emerald-500 hover:bg-emerald-600" : ""}
