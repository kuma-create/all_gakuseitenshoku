...
type FormProps = {
  ...
  showPW: boolean; loading: boolean; error: string | null;
  setEmail: (v: string) => void; setPassword: (v: string) => void;
  togglePW: () => void; onSubmit: (e: FormEvent) => void;
};
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <IconInput
          id={`${idPref}-email`} type="email" placeholder="your@mail.com"
          value={email} onChange={setEmail} icon={<Mail size={18} />}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <IconInput
            id={`${idPref}-pw`} type={showPW ? "text" : "password"} placeholder="••••••••"
            value={password} onChange={setPassword} icon={<Lock size={18} />}
          />
...
export default function LoginClient() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center
      ...
      <motion.div className="absolute bottom-0 right-0 h-72 w-72 rounded-full
      ...
      >
        ...
        <LoginForm
          role={tab} email={email} password={password}
          showPW={showPW} loading={loading} error={ctxError}
          setEmail={setEmail} setPassword={setPassword}
          togglePW={() => setShowPW((p) => !p)} onSubmit={handleLogin}
        />

        {/* signup link（next を継承） */}
        <div className="mt-8 text-center text-sm text-gray-600 dark:text-gray-400">
          アカウントをお持ちでない方は
          <Link href={`/signup${nextPath ? `?next=${encodeURIComponent(nextPath)}` : ""}`}
