...
alter table "auth"."users" alter column "role" set default 'authenticated'::text;
...
CREATE TRIGGER trg_create_student_profile AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION f_create_student_profile();

CREATE TRIGGER trg_sync_last_sign_in AFTER UPDATE OF last_sign_in_at ON auth.users FOR EACH ROW EXECUTE FUNCTION sync_last_sign_in();

CREATE TRIGGER trg_sync_user_roles AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION sync_user_roles();
