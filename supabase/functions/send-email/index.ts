...
/* ------------------------------------------------------------------ */ /* SendGrid helper                                                    */ /* ------------------------------------------------------------------ */ async function sendEmail(to, subject, html, text) {
  const body = {
    personalizations: [
      {
        to: [
...
/* ------------------------------------------------------------------ */ /* Edge Function entry                                                */ /* ------------------------------------------------------------------ */ serve(async (req)=>{
  ...
  let { id, user_id, title, message, from_role, notification_type, related_id, company_name, job_title, offer_range, job_name } = rec ?? {};
  // company_name が空文字や undefined の場合は "企業" にフォールバック
  const companyLabel = company_name && company_name.trim() !== "" ? company_name.trim() : "企業";
  // -------------------------------------------------------------
  // URL for student to view the specific scout / offer
  // -------------------------------------------------------------
  const offerLink = related_id ? `${APP_BASE_URL}/offers/${related_id}` : APP_BASE_URL;
  // ------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------
  if (user_id) {
    const { data: mapRow, error: mapErr } = await db.from("student_profiles").select("auth_user_id, full_name").eq("id", user_id).maybeSingle();
  ...
  if (!related_id) {
    ...
    return new Response("related_id required", {
      status: 400,
      headers: corsHeaders
    });
  }
  ...
  const fallbackTitle = title ?? (isScout ? "スカウト通知" : isFromCompany ? `${companyLabel}からチャット通知` : "チャット通知");
  try {
    /* 1. fetch auth user's email */ const { data, error: adminErr } = await db.auth.admin.getUserById(user_id);
    ...
    // ------------------------------------------------------------------
    const subject = isScout ? `${companyLabel} からスカウトが届きました` : isFromCompany ? `${companyLabel}から新しいチャットが届きました` : title ?? "お知らせ";
    const html = isScout ? `
        ...
        ${job_name ? `<p style="font-size:15px;margin:6px 0;"><strong>求人名:</strong> <span style="color:#d97706;font-weight:bold;">${job_name}</span></p>` : ""}

        ${job_title ? `<p style="font-size:15px;margin:6px 0;"><strong>ポジション:</strong> <span style="color:#d97706;font-weight:bold;">${job_title}</span></p>` : ""}

        ${offer_range ? `<p style="font-size:14px;margin:6px 0;"><strong>オファー額レンジ:</strong> ${offer_range}万円</p>` : ""}
    ...
    console.error("send-email error:", typeof err === "object" ? JSON.stringify(err, null, 2) : String(err));
