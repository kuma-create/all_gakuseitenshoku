/* ------------------------------------------------------------------------
   app/(auth)/s-ipo-signup/page.tsx
   - STEP1: 基本情報入力（姓・名）
   - STEP2: 確認メール送信完了
      ...
      <div className="container mx-auto max-w-screen-lg px-4 py-8">
        ...
        <div className="mb-6">
          <Link
            ...
            className="inline-flex items-center gap-2 text-sm text-gray-600 transition-colors hover:text-[#1B0718]"
          >
            <ArrowLeft size={16} />
            <span>ページに戻る</span>
          </Link>
        ...
        <div className="grid items-start gap-8 md:grid-cols-5">
          ...
          <div className="md:col-span-3">
            <Card className="border-0 shadow-lg">
              <CardHeader className="bg-gradient-to-r from-[#1B0718] to-[#120510] text-white">
                  ...
                  <form onSubmit={handleSignup} className="space-y-6">
                    ...
                    <div className="grid gap-2">
                      ...
                      <select
                        ...
                        onChange={handleInputChange}
                        className="flex h-10 w-full rounded-md border px-3 py-2 text-sm focus-visible:ring-2 focus-visible:ring-[#1B0718]"
                      >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Label htmlFor="terms" className="text-sm">
                        <Link
                          ...
                          className="underline text-[#1B0718] hover:text-[#120510]"
                    ...
                    <Button
                      type="submit"
                      className="w-full bg-[#1B0718] hover:bg-[#120510]"
...
function BenefitsSidebar() {
  const benefits = [
    { title: "メール認証", desc: "アカウントを登録するをクリック後、メール認証を行ってください" },
    { title: "職務経歴書の作成", desc: "書類審査を行いますので、長期インターンやアルバイトのご経験を職務経歴書に入力してください" },
    { title: "書類審査", desc: "職務経歴書をもとに書類審査を行い、2~3日を目安にご返信いたします。" },
  ];

  return (
    <aside className="sticky top-4 md:col-span-2">
      <div className="rounded-xl border bg-white px-6 py-8 shadow-lg">
        <h3 className="mb-6 text-lg font-bold text-gray-900">登録の流れ※重要</h3>
        <ul className="space-y-6">
          {benefits.map((b, idx) => (
            <li key={b.title} className="flex gap-4">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#1B0718]/10 text-[#1B0718] font-semibold">
                {idx + 1}
              </div>
...
function IPOPromoCard() {
  return (
    <div className="mt-6 rounded-xl border bg-white px-6 py-8 shadow-lg">
      <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-[#1B0718]/10 px-3 py-1 text-xs font-semibold text-[#1B0718]">
        コミュニティ
      </div>
      ...
      <ul className="mb-4 list-disc space-y-1 pl-5 text-sm text-gray-600">
        <li>ES添削・模擬面接（無制限）</li>
        <li>ビジネスゲームでロジカル思考を強化</li>
        <li>選抜メンバー同士のSlack交流</li>
      </ul>
      <Link
        href="https://www.internsummit.com/IPO/top"
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex w-full items-center justify-center rounded-md bg-[#1B0718] px-4 py-2 text-sm font-medium text-white hover:bg-[#120510]"
      >
        参加ページを見る
      </Link>
    </div>
