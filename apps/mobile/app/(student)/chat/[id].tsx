...
export default function ChatDetailScreen() {
  ...
  // キーボード表示状態（ここで先に定義しておく）
  const [isKeyboardOpen, setIsKeyboardOpen] = useState(false);
  // タブ配下でなくても最下部に十分な余白を確保（タブ想定 56px + セーフエリア or 最低8px）
  const bottomReserve = 56 + Math.max(insets.bottom || 0, 8);
  // さらに見た目のスキマを少し増やす（タブと入力欄が近すぎる問題の緩和）
  const extraGap = 12; // お好みで 8〜20 に調整可
  const bottomGap = bottomReserve + extraGap;
  // タブバーの見込み高さ + 少しの余白（被り回避用）
  const tabOffset = 56 + 8; // 64px
  // キーボード表示中はタブ分の余白を無効化して二重余白を防ぐ
  const dynamicTabOffset = isKeyboardOpen ? 0 : tabOffset;
  const dynamicExtraGap = isKeyboardOpen ? 4 : extraGap;
...
const styles = StyleSheet.create({
  ...
  separatorWrap: { alignItems: "center", marginVertical: 18 },
  separatorText: {
    fontSize: 13,
    color: "#374151",
  ...
  scrollToBottomText: { color: "#fff", fontWeight: "600", fontSize: 16 },
});