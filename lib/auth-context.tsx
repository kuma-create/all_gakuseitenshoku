...
interface AuthContextType {
  isLoggedIn: boolean | null // nullの場合は認証状態確認中
  userType: UserType
  user: User
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState<boolean | null>(null) // 初期値をnullに変更
    ...
    const getSession = async () => {
      try {
        ...
        if (data.session) {
          ...
          setIsLoggedIn(false) // セッションがない場合は明示的にfalseに設定
        }
      } catch (error) {
        console.error("Error getting session:", error)
        setError("セッション情報の取得に失敗しました")
        setIsLoggedIn(false) // エラー時も認証状態を確定させる
      } finally {
  ...
  useEffect(() => {
    // セッションがなく、認証状態が未確定（null）で、初期化済みの場合のみローカルストレージから復元
    if (!session && isLoggedIn === null && isInitialized) {
      try {
        console.log("Trying to restore auth state from localStorage")
        const storedAuth = localStorage.getItem("auth")
        if (storedAuth) {
          const { isLoggedIn, userType, user } = JSON.parse(storedAuth)
          console.log("Found stored auth state:", { isLoggedIn, userType })
          setIsLoggedIn(isLoggedIn)
          setUserType(userType)
          setUser(user)
        } else {
          console.log("No stored auth state found")
          setIsLoggedIn(false) // ローカルストレージにデータがない場合は明示的にfalseに設定
        }
      } catch (error) {
  ...
  }, [session, isLoggedIn, isInitialized])

  // 認証状態が変化したときにローカルストレージに保存
  useEffect(() => {
    if (isLoggedIn !== null) {
      try {
        localStorage.setItem("auth", JSON.stringify({ isLoggedIn, userType, user }))
        console.log("Auth state saved to localStorage:", { isLoggedIn, userType })
      } catch (error) {
